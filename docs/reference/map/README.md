...
These are the acceptance criteria for **gauntlet loop 1** (`docs/gauntlet-map.md`): the map
is done when the render can't be told apart from these at overview scale.
...
### `terrain_moba_2.png` — **look only, not layout** (designer, 2026-08-08)
...
working correctly — the orchestrator files them as `out-of-scope` and they do not block the
exit. See `docs/gauntlet-map.md`.
