...
## Now — M5: macro play, phase F (F1 done; F2 = multi-man plays + gank telegraph)
...
**M5-F1 is done** (designer playtest 2026-07-26, second note): the jungler now **answers** plays the map forces on him instead of only choosing his own, defending is something bodies actually do (lane presence became positional and a wave pinned on your own tower can now be killed by the players standing on it), turrets have **plating** so the first tower stops falling at 5.6 min, and the "not enough deaths" question was **measured against pro play** rather than guessed at — see `REPORTS/M5-F1.md`. 200 sims: length 27.1 → **29.0 min**, kills 22.9 → **26.3**, gank connect 17% → **22%**, tempo windows 0.65 → **1.18**, first tower 5.6 → **7.5 min**, macro win 36.0% → **34.5%** (inside noise; 33.0% on a second seed set), timeouts 0, assertions PASS.

The audit's finding is the useful part: kills-per-minute and their distribution over the clock **already matched** LCK/LEC 2025 (0.84 against 0.85). What deviates is **match length** (27 min against pro's 32 — two thirds closed) and **who dies** (mid 3.9 / carry 3.4 deaths per team-game against support 1.4 and top 1.3; pro deaths are close to flat across roles). Target selection was measured and is *not* the cause of the second one — see the report; it is now an M5-G item with a diagnosis rather than a guess.

M5-F2 is next, and E handed it a measured argument: **widening roams on a dice roll fails** (built, −3 pts macro win, no follower gain, reverted — the second time, after M5-C). Roams need the *committed* multi-man play — target, committed set, window — so the players who give up lane priority arrive together and the play is worth it.
...
- **M5-F2 — Coordinated multi-man moves + gank telegraph** (items 1 & 3). `TeamBrain` dispatches 2–3 to converge (target, committed set, window), macro-gated; the gank is telegraphed on approach and the target sets up. Enable M5-D's punish-over-extension collapse here.
